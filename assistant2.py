...
def newData(Question, Answer):
    df = pd.DataFrame({'Question': [Question],
                   'Answer': [Answer]})
    df.to_csv('assistant 2.0\Data.csv',mode='a', index=False, header=False)
...
def getAnswer(Question):
    df = pd.read_csv('assistant 2.0\Data.csv')
...
def notify(t,m):
    ...
    title = t,
    message = m,
    app_icon = None,#file path of image icon which you want to give to notification
    timeout = 5,# in seconds 1 = 1sec
)
